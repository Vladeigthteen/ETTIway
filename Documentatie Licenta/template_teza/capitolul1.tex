...
vedere al accesibilității, utilizatorul având nevoie doar de un browser modern și
...
aplicațiile care procesează date geospațiale, iar identificarea corectă a
...
secantă optimizată pentru teritoriul național. Acest sistem nu este folosit
...
geografice open-source \cite{OSM}, contribuite și întreținute de o comunitate largă de
...
\begin{figure}[htbp]
    ...
    \caption{Exemplu de aplicare a algoritmului lui Dijkstra: drumul minim de la nodul A la nodul F este evidențiat cu roșu. Algoritmul găsește traseul A → B → D → F cu cost total 9, în detrimentul alternativei mai lungi prin C și E (cost 15).}
    \label{fig:dijkstra}
\end{figure}
...
criptat între dispozitivele autorizate, izolat de traficul public. În cadrul
...
arhitectură este simplă din punct de vedere conceptual, dar suferă de două
...
\textbf{WireGuard}. Acesta este un protocol VPN deschis, conceput cu obiectivul
...
adresei externe) și \textit{hole punching} (deschiderea simultană a unor porturi
...
Astfel de servere de tranzit sunt folosite ca soluție de rezervă, traficul

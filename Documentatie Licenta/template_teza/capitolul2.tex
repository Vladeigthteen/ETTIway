...
backend. Cat și structura datelor, mecanismul de autentificare și
autorizare, sunt prezentate și fluxurile principale prin care componentele comunică între
...
interacțiunea cu utilizatorul și de calculul traseelor. Următorul nivel este
...
interfața dintre niveluri este bine definită. Un alt motiv ar fi că izolează
...
extinderea sistemului prin adăugarea de noi servicii sau de noi tipuri de date care nu
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{figuri/arhitectura_ettiway_v2.pdf}
    \caption{Arhitectura sistemului ETTIway pe trei niveluri}
    \label{fig:arhitectura-ettiway}
\end{figure}
...
structurat pe pachete distincte în funcție de responsabilități. Pachetul de
configurare conține clasele de inițializare, de securitate și de pornire a
aplicației iar pachetul de entități conține clasele care modelează datele. Pachetul de repository conține interfețele de acces la baza de
...
și inițializate fără configurare manuală explicită. In ceea ce privește codul punctului de intrare, acesta
...
de răspuns este 200 (OK), pentru resursele inexistente avem 404 (Not Found),
pentru cererile invalide este 400 (Bad Request), iar pentru tentativele de acces
neautorizat avem urmatoarele: 401 (Unauthorized) sau 403 (Forbidden). Această standardizare
...
Această abordare pe trei niveluri urmărește principiul \textit{fail fast} adică
...
\cite{PostgreSQL}, aleasă pentru suportul tipurilor diferite de date și pentru caracterul său open-source. Maparea dintre tabelele
bazei de date și obiectele din aplicație este realizată prin adnotări JPA pe
clasele de tip entitate. În cadrul ETTIway există două entități principale. Acestea sunt
...
componenta de menținere a datelor, comparând atât baze de date relaționale,
cât și soluții de tip NoSQL. În acest subcapitol vom sintetiza compromisurile
considerate și se va justifica alegerea finală.
 
Alternativa cea mai evidentă a fost o bază de date relațională clasică, fie
PostgreSQL, fie MySQL, ambele oferind maturitate, performanță și un ecosistem
...
Încă o opțiune posibilă, ar fi fost SQLite, fiind  o
...
open-source, gratuită. Mai mult, această alegere lasă deschisă o
...
Constructorul fără argumente este necesar pentru ca JPA să poată mapa
...
A doua entitate, \texttt{MapData}, este responsabilă de menținerea structurii
...
criptate folosind algoritmul BCrypt\cite{Provos1999}, prin intermediul unei
...
\begin{table}[H]
    ...
    \label{tab:utilizatori}
    \footnotesize
    \begin{tabularx}{\textwidth}{@{}c l l X@{}}
        \toprule
        \textbf{ID} & \textbf{Nume utilizator} & \textbf{Rol} & \textbf{Parolă (hash bcrypt)} \\
        \midrule
        1 & admin   & ROLE\_ADMIN & \texttt{\seqsplit{\$2a\$10\$Sj9PXuxG1OR.3E0rOO9UNOLaSoktDIGfKBzS5ehMvFlc/9E83put.}} \\
        2 & student & ROLE\_ADMIN & \texttt{\seqsplit{\$2a\$10\$fX3vR7jrl4ZiB85Ybpx/iuu9mUjSvX7XVm8hqYW/xHjpCw6ObSW...}} \\
        3 & vlad    & ROLE\_ADMIN & \texttt{\seqsplit{\$2a\$10\$GYFuTn4iOns4woYy9F1.rup7A.QeloQF1jRMWV.woK6v6GQdq.rpi}} \\
        \bottomrule
    \end{tabularx}
    \caption{Structura tabelului de utilizatori în baza de date, evidențiind
    câmpurile pentru numele de utilizator, parola criptată și rolul asociat.}
\end{table}
...
final, iar la verificare este extras automat pentru a recompune comparația.\cite{Provos1999}
...
rotunjite la o precizie fixă. Pentru a corecta micile imprecizii inerente
datelor desenate manual, vârfurile aflate la o distanță mai mică decât o
toleranță prestabilită sunt tratate ca un singur nod, ceea ce împiedică
apariția unor noduri duplicate care ar fragmenta rețeaua. În plus, segmentele
care se intersectează fără a avea un vârf comun sunt tăiate automat în punctul
de intersecție, unde se inserează un nod nou; astfel, două alei care se
încrucișează devin efectiv conectate în graf, iar rutarea poate „coti" la acea
intersecție (vezi Listing B.2 și Listing B.3).
Pentru fiecare segment rezultat se adaugă o muchie între nodurile adiacente,
ponderea muchiei fiind distanța geografică reală dintre cele două puncte.
Graful rezultat este nedirecționat, deoarece o alee poate fi parcursă în
ambele sensuri. O observație importantă privind formatul GeoJSON este că
acesta exprimă coordonatele în ordinea longitudine–latitudine, în timp ce
Leaflet le așteaptă în ordinea latitudine–longitudine. Conversia corectă între
cele două este necesară pentru ca punctele să fie plasate corect pe hartă.
...
Listing B.4 din Anexa~B). Plecând de la nodul de start, algoritmul menține
...
nodurile grafului, este necesară o etapă de asociere a fiecărui punct cu
rețeaua de trasee. Spre deosebire de o simplă apropiere de cel mai apropiat
vârf existent, punctul este proiectat pe cea mai apropiată muchie, iar în
punctul de proiecție se inserează un nod temporar conectat la capetele acelei
muchii (vezi Listing B.5). Astfel, rutarea pornește și se încheie
exact în dreptul poziției reale, nu la un capăt de segment aflat eventual
departe. Dacă între cele două puncte nu există niciun traseu ca de exemplu,
atunci când o porțiune a rețelei este izolată, algoritmul returnează o
valoare care semnalează absența unei rute, iar interfața poate informa
utilizatorul în consecință.
...
Am detaliat mecanismul de securitate, bazat pe Spring Security, cu
